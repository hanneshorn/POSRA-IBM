...
#pragma warning (default:187)
    ...
    if (atom->GetType()[0] == 'X' || atom->GetType()[0] == 'R') bracketElement = true;
    ...
    if (!atom->GetAtomicNum()) {
      AliasData* ad = (AliasData*) atom->GetData("UserLabel");
      if (ad != NULL) { // preserve X and R aliases (instead of using '*'
        string alias = ad->GetAlias();
        //cout << "SMILES alias " << alias[0] << endl;
        if      (alias[0] == 'X') strcpy(symbol,"X");
        else if (alias[0] == 'R') strcpy(symbol,"R");
        else                      strcpy(symbol,"*");
      } else if (atom->GetType() == "X" || atom->GetType() == "R") {
        //cout << "SMILES type " << atom->GetType() << endl;
        strcpy(symbol,atom->GetType());
      } else {
        //cout << "SMILES setting symbol to *" << endl;
        strcpy(symbol,"*");
      }
      //cout << "SMILES symbol is " << symbol << endl;
    } else {
      strcpy(symbol,etab.GetSymbol(atom->GetAtomicNum()));
      if (atom->IsAromatic())
        symbol[0] = tolower(symbol[0]);
    ...
    // degree e.g. [Po:n] HH
    if (atom->HasData("Degree")) {
      OBPairData* degree = dynamic_cast<OBPairData*> (atom->GetData("Degree"));
      //cout << "SMILES degree is " << degree->GetValue() << endl;
      // if the last character RIGHT HERE is an H, take it off
      // This occurs when we have a polymer that has an end group
      // of an explicit Hydrogen, for instance, PEG.
      // Bryn

      //provides search strings for the while -- looks for [LvH etc
      std::string s(bracketBuffer);
      std::string FindWordLv = "LvH";
      std::string FindWordTe = "TeH";
      std::string FindWordPo = "PoH";

      // what we will replace with if finding [LvH
      std::string ReplaceWordLv = "Lv";
      std::string ReplaceWordTe = "Te";
      std::string ReplaceWordPo = "Po";

      std::size_t index;

      //while loops look through the string 3 times, replacing [**H with [**
      while((index = s.find(FindWordLv)) != std::string::npos) {
        s.replace(index,FindWordLv.length(), ReplaceWordLv);
      }
      while((index = s.find(FindWordTe)) != std::string::npos) {
        s.replace(index,FindWordTe.length(), ReplaceWordTe);
      }
      while((index = s.find(FindWordPo)) != std::string::npos) {
        s.replace(index,FindWordPo.length(), ReplaceWordPo);
      }

      // std::cout<<s<<endl;

      strcpy(bracketBuffer, s.c_str());

      strcat(bracketBuffer, degree->GetValue().c_str());
      bracketElement = true;
    }
   ...
   *
   *       We are attempting to see if we can do the same sort of re-ordering
   *       with polymeric data.
   *
