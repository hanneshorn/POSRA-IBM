...
//          bool between = bonds_within_each_other(bond, i, j, atom);
//          if (bond[j].exists) {
//            cout << "Bond " << i << " l=" << l1 << ", Bond " << j << " l=" << l2 << " angle=" << angle_between_bonds(bond, i, j, atom) << " dbb=" << dbb << " between " << between << " avg=" << avg << endl;
//          }
          ...
          if ((bond[j].exists) && (fabs(angle_between_bonds(bond, i, j, atom)) > D_T_TOLERANCE))
            {
              ...
              // bryn
              // std::cout<<dij<<" "<<max_dist_double_bond<<endl;
