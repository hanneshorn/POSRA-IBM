      ...
      if (real_atoms > MIN_A_COUNT && real_atoms < MAX_A_COUNT && real_bonds < MAX_A_COUNT && bond_max_type>0 && bond_max_type<5)
      {
            ...
            for (unsigned int i = 0; i < fragments.size(); i++)
            {
                  ...
                  if (fragments[i].atom.size() > MIN_A_COUNT)
                  {
                        ...
                        debug_log(debug_name + "_posra_2_" + count + ".plog", avg_bond_length, atom, bond);
...
*/
                              ...
                              //debug_log(debug_name + "_posra_before_skeletize.plog", avg_bond_length, atom, bond);

                              double thickness = skeletize(atom, bond, n_bond, box, THRESHOLD_BOND, bgColor, dist, avg_bond_length);

                              // HH
                              //avg_bond_length = percentile75(bond, n_bond, atom);
                              //debug_log(debug_name + "_posra_after_skeletize.plog", avg_bond_length, atom, bond);

                              remove_disconnected_atoms(atom, bond, n_atom, n_bond);

                              // HH
                              //avg_bond_length = percentile75(bond, n_bond, atom);
                              //debug_log(debug_name + "_posra_after_remove_disconnected_atoms.plog", avg_bond_length, atom, bond);

                              collapse_atoms(atom, bond, n_atom, n_bond, 3);

                              // HH
                              //avg_bond_length = percentile75(bond, n_bond, atom);
                              //debug_log(debug_name + "_posra_after_collapse_atoms.plog", avg_bond_length, atom, bond);
                              ...
                              debug_log(debug_name + "_posra_0.plog", avg_bond_length, atom, bond);

                              double max_dist_double_bond = dist_double_bonds(atom, bond, n_bond, avg_bond_length);
                              //debug_log(debug_name + "_posra_after_max_dist_double_bond.plog", avg_bond_length, atom, bond);

                              n_bond = double_triple_bonds(atom, bond, n_bond, avg_bond_length, n_atom, max_dist_double_bond);

                              //debug_log(debug_name + "_posra_after_double_triple_bonds.plog", avg_bond_length, atom, bond);
                              ...
                              //debug_log(debug_name + "_posra_after_find_dashed_bonds.plog", avg_bond_length, atom, bond);
                              ...
                              stringstream count;
                              count << res_iter;
                              debug_log(debug_name + "_posra_1_" + count.str() + ".plog", avg_bond_length, atom, bond);
                              //nick_dev end
                              ...
                              split_fragments_and_assemble_structure_record(polymer.back(),atom,n_atom,bond,n_bond,boxes,
                                          ...
                                          recognized_chars,show_learning,res_iter,verbose, debug_name, count.str());
...
#pragma omp critical
           ...
           FILE *debug_file = fopen((debug_name + "_posra_structure.slog").c_str(), "w");
...
//                        // posra: don't put out every structure - we'll putting out the "best" one later
...
//                        }
      ...
      // posra: put out only the "best" smiles
      if ((output_format == "smi" || output_format == "can") && !is_reaction) {
